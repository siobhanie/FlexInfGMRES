...
function [x,Z,V,H,e_inner,tol_v,rtilde_vec,pvec,thist,indices]=flex_gmres(A0,Av,m,b,n,mu,which,epsilon,RESTART,TOL,MAXIT,VERBOSE,X0,IJOB,A_of_zero_inv)
    ...
    %[Z,V,H,thist]=pc_arnoldi_fac2(Av,n,m,b,mu,tol_v);
    e = zeros(m+1,1); e(1) = 1;
...
%This one is NOT for timing purposes 
    ...
    for j=1:m
        ...
        tol=1;
        if which==1
            z=fastMatSolveK_alt(A0,Av,V(:,j),j,e_inner(end),RESTART,TOL,MAXIT,VERBOSE,X0,IJOB,1);
            P=V(1:length(z),j)-fastMatVecK(Av,z,j,n);
            if norm(P)*rtilde_n<epsilon
                indices(j)=1; 
            else 
                z=fastMatSolveK_alt(A0,Av,V(:,j),j,e_inner(end),RESTART,TOL,MAXIT,VERBOSE,X0,IJOB,0); 
                P=V(1:length(z),j)-fastMatVecK(Av,z,j,n);
                indices(j)=0; 
            end
        elseif which==2
